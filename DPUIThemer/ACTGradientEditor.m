...
@synthesize action, gradient, editable, gradientHeight, drawsChessboardBackground;
...
{
	if (_editingKnobAtIndex != -1) {
		[self _setColor:color forKnobAtIndex:_editingKnobAtIndex];
	}
}
    ...
    if ([theEvent clickCount] == 2) {
        ...
        for (i = 0; i < [self.gradient numberOfColorStops]; i++) {
            ...
            if (pointsWithinDistance([self _viewPointFromGradientLocation: location], mouseLocation, kKnobDiameter)) {
				...
				[self.delegate selectedColorAtLocation:i];
